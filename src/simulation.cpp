...
Given the coordinates of two points (x, y, z) and (x1, y1, z1), this function
calculates and returns the Euclidean distance between them.
This is used to clculate the distance between cluedo_link and the waypoints.
...
@param z1 The z-coordinate of the second point.
...
/**
@brief Callback function for the Oracle node.
This function is a callback for the '/gazebo/link_states' topic and generates an
ErlOracle message containing a random hint, if the 'cluedo_link' is sufficiently
close to one of the waypoints.
 The message is published to the '/oracle' topic.

@param msg A constant pointer to the received LinkStates message.
*/
   ...
   for(int i=0; i< msg->name.size(); i++){
	   if (msg->name[i].find("cluedo_link")!= std::string::npos){
		   for(int j=0; j<4;j++){
				...
				// 1 in 4 times the generated hint is defective
				if(rand()%4==1){
					int a = rand()%5;
					if(a==0){
						oracle_msg.key = "";
					...
					bool existing = false;
					// check if the hint already exists in the msgs array
